...
## 1i moving toward using lda coefs rather than noise
...
## cov between reduced coefs and mixed partial
require(mvtnorm)
source('misc.R')
source('delong/misc.R')
set.seed(1)
...
Sigma <- matrix(runif(p^2),p)
Sigma <- Sigma%*%t(Sigma) + diag(p)
beta <- gamma <- c(runif(p.red),rep(0,p-p.red))
mu.0 <- rep(0,p)
mu.1 <- as.numeric(Sigma%*%beta)
mu.1.red <- mu.1[1:p.red]
Sigma.red <- Sigma[1:p.red,1:p.red]
params <- list(mu.0=mu.0,mu.1=mu.1,Sigma=Sigma)
## solve(Sigma.red)%*%mu.1.red
cov.beta.hajek <- vcov.full.hajek.lda(beta,params)[1:p.red,1:p.red]
a <- runif(p)#%*%solve(Sigma)
b <- runif(p)
observed <- replicate(3e2, {
    cat('.')
    terms <- replicate(1e2, {
        ...
        ## rbind(hajek=hajek.lda.deriv(x.0,x.1,beta,params), coefs=as.numeric(solve(Sigma)%*%(colMeans(x.1)-colMeans(x.0))))
        ## rbind(hajek=hajek.lda.deriv(x.0,x.1,beta,params), coefs=as.numeric(coefs.lda(x.0,x.1,params=list(Sigma=Sigma))))
        coefs <- coefs.lda(x.0[,1:p.red],x.1[,1:p.red],params=list(Sigma=Sigma.red)) 
        rbind(hajek=hajek.lda.deriv(x.0,x.1,beta,params), coefs=as.numeric(c(coefs,rep(0,p-p.red)))) 
    })
    cov(t(a%*%terms['coefs',,]), t(b%*%terms['hajek',,]))
})
hist(observed)
abline(v=a[1:p.red]%*%cov.beta.hajek%*%b[1:p.red] * (1/m+1/n),col=2)
abline(v=mean(observed),col=3)
...
## combining the above simulations to build full vcov
## matrix. #1 cov between reduced coefs (betahat) and hajek, #2 cov
## between #full coefs (gammahat) and hajek, #3 variance of full
## coefs, #4 var #of reduced coefs, #5 cov between reduced and full
## coefs (betahat #and gammahat), #6 full vcov matrix for full and
## reduced coefs, #7 var of hajek part, #8 full vcov matrix for
## reduced coefs (betahat, full coefs (gammahat) and hajek part
require(mvtnorm)
source('misc.R')
source('delong/misc.R')
## set.seed(1)
...
observed <- replicate(3e2, {
    ...
    terms <- replicate(1e2, {
        x.0 <- rmvnorm(m,mu.0,Sigma)
        x.1 <- rmvnorm(n,mu.1,Sigma)
        term.a <- coefs.lda(x.0[,1:p.red],x.1[,1:p.red],params=list(Sigma=Sigma.red)) #1, #4, #5, #6, #7, #8
        term.a <- as.numeric(c(term.a,rep(0,p-p.red))) #1, #4, #5, #6, #7, #8
        term.b <- hajek.lda.deriv(x.0,x.1,beta,params) #1, #7, #8
        ## term.a <- as.numeric(coefs.lda(x.0,x.1,params=list(Sigma=Sigma))) #2,#3
        ## term.b <- hajek.lda.deriv(x.0,x.1,beta,params) #2
        ## term.b <- as.numeric(coefs.lda(x.0,x.1,params=list(Sigma=Sigma))) #5, #6
        rbind(a=term.a, b=term.b) #1,#2,#5
        ## term.a #3
        ## term.a #4
        ## term.b #7
        ## c(term.a,term.b) #6
        term.c <- as.numeric(coefs.lda(x.0,x.1,params=list(Sigma=Sigma))) #8
        c(term.a, term.c, term.b) #8
    })
    ## cov(t(a%*%terms['a',,]), t(b%*%terms['b',,])) 
    ## var(t(terms)%*%a) 
    ## var(t(terms)%*%c) #6
    var(t(terms)%*%d) #8
})
hist(observed)
## abline(v=a%*%cov.beta.hajek%*%b ,col=2) # 1
## abline(v=a%*%cov.gamma.hajek%*%b ,col=2) #2
## abline(v=a%*%var.gamma%*%a ,col=2) #3
## abline(v=a%*%var.beta%*%a ,col=2) #4
## abline(v=a%*%cov.beta.gamma%*%b ,col=2) #5
## abline(v=c%*%vcov.beta.gamma%*%c, col=2) #6
## abline(v=a%*%var.hajek%*%a, col=2) #7
abline(v=d%*%vcov.beta.gamma.hajek%*%d, col=2) #8
abline(v=mean(observed),col=3)
...
## 1j sampling to approximate the asy distribution of target. using
## lda coef estimatess
require(mvtnorm)
require(parallel)
source('misc.R')
...
mu.1 <- as.numeric(Sigma%*%beta)
params <- list(mu.0=mu.0,mu.1=mu.1,Sigma=Sigma)
vcov.beta.gamma.hajek  <-  vcov.lda(beta,p.red,params) * (1/m+1/n)
auc.deriv2 <- auc.scores.deriv2.lda(beta,Sigma.diff=2*Sigma)
obs <- replicate(5e2, {
    x.0 <- rmvnorm(m,mu.0,Sigma)
    x.1 <- rmvnorm(n,mu.1,Sigma)
    beta.hat <- coefs.lda(x.0[,1:p.red],x.1[,1:p.red],params=list(Sigma=Sigma[1:p.red,1:p.red]))
    beta.hat <- as.numeric(c(beta.hat,rep(0,p-p.red))) 
    gamma.hat <- as.numeric(coefs.lda(x.0,x.1,params=list(Sigma=Sigma))) 
    ## beta.hat <- beta+epsilon.beta.gamma[1:p]#rnorm(p)/n^.5
    ## gamma.hat <- beta+epsilon.beta.gamma[(p+1):(2*p)]#rnorm(p)/n^.5
    ## terms <- sapply(list(beta.hat,gamma.hat), function(beta.hat) {
    ## })
    ## -n * diff(terms)
    auc.hat(x.0%*%beta.hat,x.1%*%beta.hat) - auc.hat(x.0%*%gamma.hat,x.1%*%gamma.hat)  
})
plot(ecdf(obs))
B <- 1e3
beta.gamma.hajek.star <- rmvnorm(B,sigma=vcov.beta.gamma.hajek)
beta.star <- beta.gamma.hajek.star[,1:p]
gamma.star <- beta.gamma.hajek.star[,(p+1):(2*p)]
hajek.star <- beta.gamma.hajek.star[,(2*p+1):(3*p)]
reference.hajek <- rowSums((beta.star-gamma.star)*hajek.star)
reference.taylor <-   (rowSums((beta.star%*%auc.deriv2)*beta.star)/2 - rowSums((gamma.star%*%auc.deriv2)*gamma.star)/2)
plot(ecdf( (reference.hajek+reference.taylor)),add=TRUE,col=2)


## 1k estimation

## 1k-1 estimation of taylor hessian. numDeriv::hessian haywire with
## components corresponding to beta==0
...
x.0 <- rmvnorm(m,mu.0,Sigma)
x.1 <- rmvnorm(n,mu.1,Sigma)
## beta.hat <- coefs.lda(as.matrix(x.0[,1:p.red]),as.matrix(x.1[,1:p.red]),params=list(Sigma=Sigma[1:p.red,1:p.red]))
## beta.hat <- as.numeric(c(beta.hat,rep(0,p-p.red))) 
## gamma.hat <- as.numeric(coefs.lda(x.0,x.1,params=list(Sigma=Sigma))) 
## auc.hat(x.0%*%beta.hat,x.1%*%beta.hat) 
beta.hat <- beta + rnorm(p)/sqrt(n)
auc.deriv2 <- auc.scores.deriv2.lda(beta,Sigma.diff=2*Sigma)
numDeriv::hessian(function(beta)auc.hat(x.0%*%beta,x.1%*%beta),beta)
## pracma::hessian(function(beta)auc.hat(x.0%*%beta,x.1%*%beta),beta.hat)

auc.deriv2
numDeriv::hessian(function(beta)auc.lda(beta,params),x=beta.hat) - auc.deriv2


numDeriv::grad(function(beta)auc.hat(x.0%*%beta,x.1%*%beta),x=beta.hat,method='simple',method.args=list(eps=1/n^(.5)))
numDeriv::grad(function(beta)auc.hat(x.0%*%beta,x.1%*%beta),x=beta.hat)


ts <- seq(-1,1,len=30)
a <- runif(p); b <- runif(p)
aucs <- sapply(ts, function(t)auc.hat(x.0%*%(beta+a*t),x.1%*%beta+b*t))
plot(ts,aucs)
abline(v=0)
...
beta.hat <- beta + rnorm(p)/sqrt(n)
gamma.hat <- beta + rnorm(p)/sqrt(n)
## qform.hat <- as.numeric(t(beta.hat)%*%Sigma%*%beta.hat)
true <- as.numeric(dnorm((as.numeric(x.1%*%gamma) - t(beta)%*%mu.0)/sqrt(qform))) * x.1/sqrt(qform)
h0 <- hist(x.0%*%beta.hat)
breaks <- h0$breaks
breaks <- c(-Inf,breaks,Inf)
vals <- c(0,h0$density,0)
cuts <- cut(x.1%*%gamma.hat,breaks)
approx <- vals[cuts] * x.1
a <- runif(p)
plot(ecdf(true%*%a))
plot(ecdf(approx%*%a),add=TRUE,col=2)


## harder partial. requiring p=1
...
x.0 <- rmvnorm(m,mu.0,Sigma)
x.1 <- rmvnorm(n,mu.1,Sigma)
beta.hat <- beta + c(rnorm(1)/sqrt(n),0)
gamma.hat <- beta + rnorm(p)/sqrt(n)
## beta.hat <- gamma.hat <- beta
## true <- as.numeric(dnorm((as.numeric(x.1%*%gamma) - t(beta)%*%mu.0)/sqrt(qform))) * x.1/sqrt(qform)
inner <- (as.numeric(x.1%*%gamma) - t(beta)%*%mu.0)/sqrt(qform)
true <- as.numeric(dnorm(inner)) * (-mu.0/sqrt(qform) - (inner/qform)%*%(t(beta)%*%Sigma))
true <- true[,1]
true <- - as.numeric(dnorm(inner)) * x.1%*%gamma/ sqrt(qform)/beta[1]
h0 <- hist(x.0[,1],breaks='FD')
breaks <- h0$breaks
breaks <- c(-Inf,breaks,Inf)
vals <- c(0,h0$density,0)
cuts <- cut(x.1%*%gamma.hat / beta.hat[1],breaks)
approx <- -vals[cuts] * x.1%*%gamma.hat / beta[1]^2
## a <- runif(p)
plot(ecdf(true))
plot(ecdf(approx),add=TRUE,col=2)
## hist(x.0[,1],prob=TRUE)
## curve(dnorm((x-mu.0[1])/sqrt(Sigma[1,1]))/sqrt(Sigma[1,1]),add=TRUE)
## true <- - as.numeric(dnorm(inner)) /sqrt(Sigma[1,1])
## h0 <- hist(x.0[,1])
## breaks <- h0$breaks
## breaks <- c(-Inf,breaks,Inf)
## vals <- c(0,h0$density,0)
...
x.0 <- rmvnorm(m,mu.0,Sigma)
x.1 <- rmvnorm(n,mu.1,Sigma)
beta.hat <- beta + c(rnorm(1),0)/sqrt(n)
gamma.hat <- beta + rnorm(p)/sqrt(n)
## beta.hat <- gamma.hat <- beta
## true <- as.numeric(dnorm((as.numeric(x.1%*%gamma) - t(beta)%*%mu.0)/sqrt(qform))) * x.1/sqrt(qform)
## inner <- (as.numeric(x.1%*%gamma) - t(beta)%*%mu.0)/sqrt(qform)
...
## true <- true[,1]
## true <- - as.numeric(dnorm(inner)) * x.1%*%gamma/ sqrt(qform)/beta[1]
true <- dnorm((x.1%*%beta - beta[1]*mu.0[1]) / sqrt(qform)) / sqrt(Sigma[1,1]) * x.1[,2]/beta[1]
## true <- true %*% c(-beta[2]/beta[1],1)
...
## harder partial, not requiring p=1 this time
...
Sigma <- Sigma%*%t(Sigma) #+ diag(p)
## Sigma <- diag(p)
beta <- gamma <- c(runif(p.red,0,1),rep(0,p-p.red)) 
qform <- as.numeric(t(beta)%*%Sigma%*%beta)
mu.0 <- rep(0,p)
mu.1 <- as.numeric(Sigma%*%beta)
params <- list(mu.0=mu.0,mu.1=mu.1,Sigma=Sigma)
...
true <- true[,1]
## true <- - as.numeric(dnorm(inner)) * x.1%*%gamma/ sqrt(qform)/beta[1]
h0 <- hist(x.0[,1],,plot=FALSE,breaks=1e3)#'FD')
breaks <- h0$breaks
breaks <- c(-Inf,breaks,Inf)
f.vals <- c(0,h0$density,0)
inner <- x.1%*%gamma.hat - rowSums((x.0%*%beta.hat)[,-1])
cuts <- cut( inner/beta.hat[1] , breaks)
approx <- -f.vals[cuts] * inner/beta.hat[1]^2
## a <- runif(p)
plot(ecdf(true))
plot(ecdf(approx),add=TRUE,col=2)

...
if(length(args)==0) B <- 1e2 else B <- as.numeric(args[1])
## B <- 1e4
alpha <- .05
...
for(i in 1:B) {
    ...
    Delta.rho <- tryCatch(auc.to.params(theta.12=theta.12[i],theta.11=theta.11[i]),
                          error=function(e) e)
                          ##     cat('no sign change: ',theta.11[i],', ',theta.12[i],'\n')
    ## })
    if('error' %in% class(Delta.rho)) { cat('no sign change: ',theta.11[i],', ',theta.12[i],'\n'); next}
...
# for n in $(seq 1 20); do ./submit.sh sim.R 1e2; done

...
png('211120.png')
plot(0,type='n',xlim=c(1/2,1),ylim=c(1/2,1),xlab=expression(theta[11]),ylab=expression(theta[12]),asp=1)
contour(theta.11,theta.12,power,add=TRUE,nlevels=8,method='simple')
## contourplot(power)
abline(a=0,b=1)
text(.73,.75,expression(paste(H[0]:~"|"~theta[11]-1/2~"|=|"~theta[12]-1/2~"|")),srt=45)
dev.off()

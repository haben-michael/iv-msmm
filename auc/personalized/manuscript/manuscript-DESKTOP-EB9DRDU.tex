...
completely uninformative, $\aucpop=1/2$, the above bounds on the individual AUC, which are tight, are vacuous, $0\le\aucindiv\le 1$. Situations as described in Section \ref{section:examples}, where the population AUC $\to 1$ while the individual AUC $\to 1/2$, appear to require some dependence between $M,N$ and $X,Y$. % Though we do not completely prove this below, giving the bounds not for any $(M,N)\cind (X,Y)$ but specifically for constant $M,N$.
...
individual AUCs of $\aucindiv\in\{\input{sim_coverage_theta.11s.txt}\}$ with $\aucindiv\ge\aucpop$. For each setting of $\rho_{MN},\aucpop,\aucindiv$, $\input{sim_coverage_reps.txt}$ replicates
...
\ref{table:1}. The bias is on the order of a hundredth at this sample size, and the coveage is generally close to .95. There is some degradation in the coverage as $(\aucpop,\aucindiv)$ approach $(1,1)$.
...
number of clusters is $\I=10$, few relative to the setting in
...
\input{input/da_total_stops_nyc.txt} stops carried out in between 2017
...
\item $|\aucpop-1/2|<|\aucindiv-1/2|$. We next consider duration of
  ...
  \input{input/da_white_boston_pval.txt}. This
  analysis therefore corresponds to the situation in
  Fig. \ref{fig:comparison:a}.

\item No significant difference between $|\aucpop-1/2|$ and
  $|\aucindiv-1/2|$. Finally, we consider duration of the stop between non-Hispanic (``control'') and Hispanic (``case'') suspects. For both the NYC and Boston data, neither the
...
The decision to cluster at the officer or command level, as opposed
...
assumption further impose modelling assumptions such as linear
...
for mulitiple dependent AUCs, clusters that are only exchangeable or
...
strong enough assumption such as ignorability, ie, the behavior of the

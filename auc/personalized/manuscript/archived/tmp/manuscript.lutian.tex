...
\end{aligned}
\end{gather}
The control and case observations in a cluster, $X_i$ and $Y_i$, are then those $B_i$ such that $D_i=0$ and $D_i=1$, respectively. Here the cluster allocations $M$ and $N$ and the markers $\vec{B}$ are dependent, both being functions of $Z$ and $\vec{\xi}$, though they are conditionally independent given the statuses $\vec{D}$. Note that we only consider clusters with both $M$ and $N$ being positive. The following computations ignore this condition, which is justifiable when the bernoulli probability $\sigma(\beta_0Z+\beta_1\xi_i)$ are not too close to 1 or 0 for at least some $i$s.
...
random variables $M,N$, such that $P( M> 0)=P(N> 0)=1$,
$\E (M^{-2})<\infty,\E (N^{-2})<\infty$.  Note $M$ and $N$ oftentimes are but need not be the
lengths of $X$ and $Y$, respectively.  Let $\W{i}=(X_i,Y_i,M_i,N_i),i=1,\ldots,\I,$ and let $\seqspace$ denote the space of finite sequences.
...
\end{align}
has a chi-squared distribution with 2 degrees of freedom. Therefore, if
$q_\alpha$ is an upper $\alpha$ quantile of this distribution, then
$$
\left\{\begin{pmatrix}x\\y\end{pmatrix}:\bigg\vert \hat{\Sigma}^{-1/2}\left(\begin{pmatrix}x\\y\end{pmatrix}-\begin{pmatrix}\aucpophat\\\aucindivhat\end{pmatrix}\right)\bigg\vert^2
< q_\alpha\right\}
$$
is a level $1-\alpha$ confidence region for $(\aucpop,\aucindiv)$, which then
covers $(\aucpop,\aucindiv)$ when \eqref{eqn:simulation:confidence region} is $<q_\alpha$. Here, $\hat\Sigma$  is a consistent estimator $\Sigma$ given in Corollary \ref{corollary:variance estimator}.
% This process was repeated $[[...]]$ times.
Results are presented in Table
\ref{table:1}. The bias is on the order of a hundredth at this sample size, and the coverage is generally close to .95. There is some degradation in the coverage as $(\aucpop,\aucindiv)$ approach $(1,1)$. This is expected, since the large sample approximation becomes inaccurate, when the true parameters are close to the boundary of the parameter space.
...
\begin{table}
 ...
 % \caption{}
  ...
  \\
    \subfloat[Bivariate normal model with censoring \eqref{model:censored binormal}]{\input{220811.tex}}
      %       \label{table:2}
    \caption{The results of a simulation examining the coverage of a nominal 95\% confidence ellipse obtained using the asymptotic estimator given in Section \ref{section:asymptotics}. %\comment{make conssitent order of theta11 vs theta12 columns}. 
    For $\aucindiv$ and $\aucpop$, the bias is computed as the mean difference between the estimates and the known true values. For the elements of the covariance matrix $\Sigma_{ij}$, the bias is the mean difference between the estimates given by Theorem \ref{theorem:asymptotic}   and the empirical covariance.}
    \label{table:1}
\end{table}
...
\begin{figure}[!tbp]
  \centering
  \subfloat[Bivariate normal model \eqref{model:binormal}]{\includegraphics[width=0.5\textwidth]{220813a.pdf}}
  \hfill
  \subfloat[Bivariate normal model with censoring \eqref{model:censored binormal}]{\includegraphics[width=0.5\textwidth]{220813b.pdf}}

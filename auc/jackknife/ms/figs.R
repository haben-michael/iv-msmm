...
## png('fig1.png')
plot(0,type='n',xlim=range(Is),ylim=c(0.0,1.5),xlab='sample size',ylab=expression('(sample size)'^2%*%'(difference of variance estimates)'),mgp=c(2,1,0))
## op <- par(mfrow=c(1,2))
...
## par(op)
legend('topright',lty=rep(1:length(by.cluster.size),each=2),pch=rep(c(NA,1),2),cex=.7,pt.cex=.7,legend=paste0(rep(paste0('m=n=',cluster.sizes),each=2),c(', jackknife',', 2-sample jackknife')))
## dev.off()
...
Q <- M.obj(3,sym=TRUE)
png('fig6.png')
image2(Q,col=hcl.colors(12,"Grays",rev=0),xaxt='n',yaxt='n')
dev.off()




## ratio of positive and negative parts
source('../misc.R')
Q <- M.obj(3,sym=TRUE)
image2 <- function(m,...)image(t(m)[,nrow(m):1],...)
...
unique(round(c(neg.qf/pos.qf),5))
png('fig8.png')
image(log(abs(neg.qf/pos.qf)),col=hcl.colors(12,"Grays",rev=0),xaxt='n',yaxt='n')
dev.off()
...
## Magnitudes of negative and positive parts
source('../misc.R')
require(parallel)
I <- 5
...
png('fig7.png')
plot(qfs['pos',],qfs['neg',],xlab=expression(phi^t*Q[1]*phi),ylab=expression(phi^t*Q[2]*phi),mgp=c(2,1,0))
abline(0,1)
dev.off()

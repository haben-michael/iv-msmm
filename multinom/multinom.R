...
## ## 2b encapsulate routine using s3 class. [update: I was planning to
## ## refactor multinom.linear to allow for this type of sampling
## ## (sampling p and then deriving theta rather than specifying theta
## ## then sampling p's for each theta), but this approach seemed too
## ## conservative. maybe i didnt find the right parameter setting, but
## ## that would cause trouble when providing guidance to
## ## practitioners. so for now planning to stick with the other sampling
## ## approach.]

## multinom.linear.lu <- function(p.obs,c,n,theta,n.ref.samples=1e2,test.stat,p.sampler,theta.resolution=NULL,p.resolution=NULL,epsilon=1e-1) {
##     ## browser()
##     m <- length(c)
##     p <- matrix(rexp(m*p.resolution),ncol=m)
##     p <- p / rowSums(p)
##     if(sum(abs(rowSums(p)-1)>1e-6)>1)browser()
##     ## p.val <- apply(p,1,p.val.mc,p.obs=p.obs,n=n,n.ref.samples=n.ref.samples,c=c,T=test.stat)
##     theta.to.p <- lapply(theta, function(theta.i) which(abs(p%*%c - theta.i)<epsilon))

##     idx.needed <- unique(unlist(theta.to.p))
##     p.val <- rep(NA,nrow(p))
##     p.val[idx.needed]  <-  apply(p[idx.needed,,drop=FALSE],1,p.val.mc,p.obs=p.obs,n=n,n.ref.samples=n.ref.samples,c=c,T=test.stat)
##     ## if(is.null(theta))theta <- seq(min(c),max(c),len=theta.resolution)
##     ## p.val.by.theta <- lapply(theta, function(theta.i) {
##     ##     idx <- which(abs(p%*%c - theta.i)<epsilon)
##     ##     if(length(idx)==0) NA else p.val[idx]#p.val[which(abs(p%*%c - theta.i)<epsilon)])
##     ## }
##     ## max.p.vals <- sapply(p.val.by.theta, max)

##     max.p.vals <- sapply(theta.to.p, function(idx) if(length(idx)>0) max(p.val[idx]) else NA)
    
##     out <- structure(list(theta=theta,p=p,p.val.p=p.val,p.val.theta=max.p.vals,theta.to.p=theta.to.p,data.name=paste(deparse(substitute(p.obs))),p.obs=p.obs), class='multinom.linear')
##     return(out)
## }
## ## coverage sim
## require(parallel)
...
##             ml <- multinom.linear.lu(p.obs=x/n,c=c,n=n,theta=theta.true,n.ref.samples=5e2,test.stat=test.stat.1,p.sampler=p.sampler.2,theta.resolution,p.resolution=1e4,epsilon=.01)
...
## 4 new sampling strategy (fast but nonuniform sampling)
...
## ## 6 sampling method using vertex enumeration (slow but uniform
## ## sampling. actually can be dirichlet)
...
## ## 6a vertices of the original problem, ignoring orthcomplement etc (update: didn't pan out)
## require(vertexenum)
## require(rgl)
## m <- 4
...
## ## 6b transforming intersection to simplex, sampling, transforming back
...
## ## 6c transforming intersection to simplex, sampling, transforming back,
...
## ## 6d checking p value distribution is uniform
...
##     x <- rmultinom(1,n,p.true)
##     ## cover.exact <- test.coverage.exact(theta.try=theta.true,p.hat=x/n,c=c,n=n,alpha=alpha,n.ref.samples=5e1,test.stat=test.stat.1,p.sampler=p.sampler.2,theta.resolution=20,p.resolution=2,shape=1)
##     ml <- multinom.linear(p.obs=x/n,c=c,n=n,theta=theta.true,n.ref.samples=2e3,test.stat=test.stat.1,p.sampler=p.sampler.1,theta.resolution=NULL,p.resolution=1)
##     test.multinom.linear(ml,theta.null=theta.true)$p.val 
## })
## ## })
## ## })
## ## by.n <- simplify2array(by.n)
...
## a <- 1:5
## p <- runif(length(a)); p <- p/sum(p)
## pvals <- replicate(1e3, {    
##     x.star <- sample(a,5e3+1,prob=p,replace=TRUE)
##     x <- x.star[1]
##     mean(x<=x.star[-1])
##     })
## plot(ecdf(pvals),xlim=c(0,1),ylim=c(0,1)); abline(0,1)
...
## p.sampler <- p.sampler.1
## theta.resolution <- 1e2
## theta <- seq(min(c)+.001,max(c)-.001,len=theta.resolution)
## theta <- seq(min(c),max(c),len=theta.resolution) 
## p.resolution <- 1e2
## n.ref.samples <- 1e2
...
## ## 8 power simulations, m=2

## ## smoothly drawing p-values on surface of 2-d simplex

## ## alternate plot routine, only useful for low-dim m
## ## plot.multinom.linear <- function(test.output){
##     theta <- test.output$theta
##     p <- test.output$p
##     p.val.p <- test.output$p.val.p
##     p <- do.call(rbind,p)
...
## xyz <- data.frame(x=p[,1],y=p[,2],p.val=p.val.p)
## palette <- heat.colors(10)
## palette <- gray.colors(10)
## n.colors <- length(palette)
## p.val.binned <- cut(xyz$p.val,breaks=n.colors)
## col <- palette[p.val.binned]
## with(xyz, plot(x,y,col=col,asp=1))

## prec <- 2
## xyz$x <- round(xyz$x,prec)
## xyz$y <- round(xyz$y,prec)
## xyz <- xyz[order(xyz$x,xyz$y),]
## ## xyz <- xyz[order(xyz$x,xyz$y),]
## xyz <- aggregate(p.val ~ x + y, FUN=mean, data=xyz, na.rm=TRUE)
## x <- y <- seq(0,1,by=10^-prec)
## xyz.full <- expand.grid(y=y,x=x)
## xyz.full <- merge(xyz.full,xyz,all.x=TRUE)
## ## xyz.full <- as.data.frame(xyz.full)
## power.mat <- array(xyz.full[,'p.val'], c(length(x),length(y)))
## ## contour(x,y,power.mat,nlevels=10,method='simple')
## ## plot(0,type='n',xlim=c(0,1),ylim=c(0,1),xaxs='i',yaxs='i')
## contour(x,y,power.mat,add=TRUE,nlevels=10,method='simple')
## ## contour(x,y,as.matrix(blur(as.im(power.mat),sigma=1)),add=TRUE,nlevels=10,method='simple')
...
## plot(0,type='n',xlim=c(0,1),ylim=c(0,1),xaxs='i',yaxs='i')
## contour(x,y,power.mat.ma,add=TRUE,nlevels=10,method='simple')


## image(power.mat,col=palette)
## contour(x,y,power.mat,add=TRUE,nlevels=5,method='simple')


## ## try knn
## k <- 5
## dist.xy <- as.matrix(dist(xyz.full[,1:2],diag=TRUE))
## dist.xy[,which(is.na(xyz.full$p.val))] <- Inf
## xyz.knn <- xyz.full
## xyz.knn$p.val <- apply(dist.xy, 1, function(r) mean(xyz.full$p.val[order(r)[1:k]]))
## power.mat <- array(xyz.knn[,'p.val'], c(length(x),length(y)))
## ## contour(x,y,power.mat,nlevels=10,method='simple')
## power.mat[row(power.mat)>ncol(power.mat)-col(power.mat)+1] <- NA
## plot(0,type='n',xlim=c(0,1),ylim=c(0,1),xaxs='i',yaxs='i')
## contour(x,y,power.mat,add=TRUE,nlevels=5,method='simple')
## ## contour(x,y,as.matrix(blur(as.im(power.mat),sigma=1)),add=TRUE,nlevels=10,metho
...
## ## ns <- round(seq(10,5e1,len=5))
## ## ns <- c(10,30,50)
## ## p.trues <- sapply(seq(.01,1/m,len=3),function(x)c(rep(x,m-1),1-(m-1)*x))
## ## ## by.n <- lapply(ns, function(n) {
## ## by.n <- mclapply(ns, mc.cores=detectCores()-3, FUN=function(n) {
## ##     cat('------')
## ##     by.p.true <- apply(p.trues, 2, function(p.true) {
...
## theta <- seq(min(c),max(c),len=theta.resolution) 
## n <- 30
## p.true <- c(rep(.1,m-1),1-(m-1)*.1)
## theta.true <- as.numeric( p.true%*%c )
## p.vals <- replicate(5e1, simplify=FALSE, expr={
##     ## browser()
##     ## cat('.')
##     x <- rmultinom(1,n,p.true)
##     ## test.coverage.asy(theta.try=theta.true,p.hat=x/n,c=c,n=n)
##     ## cover.exact <- test.coverage.exact(theta.try=theta.true,p.hat=x/n,c=c,n=n,alpha=alpha,n.ref.samples=5e1,test.stat=test.stat.1,p.sampler=p.sampler.2,theta.resolution=20,p.resolution=2,shape=1)
##     ml <- multinom.linear(p.obs=x/n,c=c,n=n,theta=theta,n.ref.samples=3e2,test.stat=test.stat.1,p.sampler=p.sampler.1,theta.resolution=NULL,p.resolution=10)
##     ## plot3d.multinom.linear(ml)
##     ## cover.exact <- test.multinom.linear(ml,theta.null=theta.true)$p.val >= alpha
##     ## if(cover.exact.1!=cover.exact)browser()
##     ## cover.exact <- test.coverage.exact(theta.try=theta.true,p.hat=x/n,c=c,n=n,alpha=alpha,n.ref.samples=5e1,T=test.stat.1,theta.resolution=200,p.resolution=20,shape=1)
##     ## cover.asy <- test.coverage.asy(theta.try=theta.true,p.hat=x/n,c=c,n=n,alpha=alpha)
...
##     p.val.p <- ml$p.val.p
##     p <- do.call(rbind,p)
##     p.val.p <- do.call(base:::c,p.val.p) # clean this clash
##     ## plot(theta,p.val.theta,ylim=c(0,1),xlab='theta',ylab='p-value')
##     ## abline(h=alpha)    
##     ## }
## ## plot.multinom.linear(test.output)
##     data.frame(x=p[,1],y=p[,2],p.val=p.val.p)
## })
## ##     })
## ## })
## ## by.n <- simplify2array(by.n)
...
## ## png('power.png', width = 1024, height = 768, pointsize=15)
## xyz <- do.call(rbind,p.vals)
## palette <- heat.colors(10)
## palette <- gray.colors(10)
## n.colors <- length(palette)
## p.val.binned <- cut(xyz$p.val,breaks=n.colors)
...
## points(p.true[1],p.true[2],pch=3,col=2,cex=3)
## abline(a=(theta.true-c[3])/(c[2]-c[3]),b=-(c[1]-c[3])/(c[2]-c[3]),col=2)
## ## dev.off()
...
## xyz <- aggregate(p.val ~ x + y, FUN=mean, data=xyz, na.rm=TRUE)
## x <- y <- seq(0,1,by=10^-prec)
## xyz.knn <- expand.grid(y=y,x=x)
## xyz.knn <- merge(xyz.knn,xyz,all.x=TRUE)

## k <- 5
## dist.xy <- as.matrix(dist(xyz.knn[,1:2],diag=TRUE))
## dist.xy[,which(is.na(xyz.knn$p.val))] <- Inf
## xyz.knn$p.val <- apply(dist.xy, 1, function(r) mean(xyz.knn$p.val[order(r)[1:k]]))
## power.mat <- array(xyz.knn[,'p.val'], c(length(x),length(y)))
## ## contour(x,y,power.mat,nlevels=10,method='simple')
## power.mat[row(power.mat)>ncol(power.mat)-col(power.mat)+1] <- NA
## plot(0,type='n',xlim=c(0,1),ylim=c(0,1),xaxs='i',yaxs='i',xlab=expression(p[1]),ylab=expression(p[2]))
## contour(x,y,power.mat,add=TRUE,nlevels=5,method='simple')
## ## contour(x,y,as.matrix(blur(as.im(power.mat),sigma=1)),add=TRUE,nlevels=10,metho

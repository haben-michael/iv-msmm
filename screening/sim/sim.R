...
## ## students t call
## for n in ${ns[@]}; do for x in $(seq 1 $reps); do for param in $(seq 2.1 .1 4); do Rscript sim.R 100 $n 1 $param; done; done; done


## ## power law call:
## for n in ${ns[@]}; for x in $(seq 1 $reps); do for param in $(seq -.9 .1 0); do Rscript sim.R 1000 $n 2 $param; done; done

## ## beta call
## for n in ${ns[@]}; for x in $(seq 1 $reps); do for param in $(seq .2 .1 1); do Rscript sim.R 100 $n 3 $param; done; done; done

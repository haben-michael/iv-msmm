...
taus <- c(0, 1, 4)
pub.rates <- list(c(1, 1), c(1, 0.05), c(1, 0.02), c(1, 0))
...
for(id.tau in 1:length(taus)){
  ...
  for(id.pub.rate in 1:length(pub.rates)){
    ...
    pvals.list <- mclapply(1:n.iter, function(i){
        sim.suppress.pval(mu, tau, s.low, s.upp, pub.rate,
    ...
    write.table(props.nonsig.smry, file =
      paste("suppress_pval_n_", n, "_tau_", tau,
    ...
    write.table(rslts, file = paste("suppress_pval_n_", n, "_tau_", tau,
      "_pub.rate.sig_", pub.rate[1], "_pub.rate.nonsig_", pub.rate[2],

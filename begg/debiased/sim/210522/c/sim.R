## checking whether term.2 is small enough when conditioning on term1<0
## P(A<0) is close to P(quadform<0) checking sym diff 
start.time <- Sys.time()
...
symdiff <- function(A,B) (A & !B) | (B & !A)  #(A | B) & !(A&B)

by.rZ <- lapply(rZs, function(rZ) {
    probs.full <- probs.1 <- symdiff.1 <- symdiff.2 <- symdiff.3  <- symdiff.3a <- symdiff.3b <- numeric()
    ...
    for(n in ns) {
        ...
        probs.full <- c(probs.full,mean(term1 + term2 < 0))
        probs.1 <- c(probs.1, mean(term1 < 0))
        ## probs.2 <- cbind(probs.2, mean(abs(term1)<abs(term2)))
        symdiff.1 <- c(symdiff.1, mean(symdiff(term1 < 0, term1+term2+theta.hat^2 < 0)))
        symdiff.2 <- c(symdiff.2, mean(symdiff(term1+term2+theta.hat^2 < 0, term1+term2 < 0)))
        symdiff.3 <- c(symdiff.3, mean(symdiff(term1 < 0, term1+term2 < 0)))
        symdiff.3a <- c(symdiff.3a, mean((term1<0) & !(term1+term2<0)))
        symdiff.3b <- c(symdiff.3b, mean((term1+term2<0) & !(term1<0)))
   }
    list(probs.full=probs.full,probs.1=probs.1,symdiff.1=symdiff.1,symdiff.2=symdiff.2,symdiff.3=symdiff.3,symdiff.3a=symdiff.3a,symdiff.3b=symdiff.3b)
})

...
by.rZ <- lapply(rZs, function(rZ) {
    probs.full <- probs.3 <- probs.2 <- probs.1 <- numeric()
    ...
    for(n in ns) {
        ...
        probs.3 <- cbind(probs.3, mean((abs(term1)<abs(term2)) & (term1 < 0)))
   }
    list(probs.full=probs.full,probs.1=probs.1,probs.2=probs.2,probs.3=probs.3)
})

...
beta <- runif(1,-1,1); gamma <- runif(1,0,1);alpha <- runif(1,0,1);
integrate(function(u)pnorm(alpha*u)*pnorm(beta*u)*dnorm(gamma*u),-Inf,Inf)$val-1/gamma/2/pi*atan(alpha*beta/gamma/sqrt(alpha^2+beta^2+gamma^2))-1/4/gamma
...
max(abs(replicate(1e2,{
    beta <- rnorm(1); gamma <- runif(1,0,1);alpha <- runif(1,0,1);
    integrate(function(u)pnorm(alpha*u)*pnorm(beta*u)*dnorm(gamma*u),-Inf,Inf)$val-1/gamma/2/pi*atan(alpha*beta/gamma/sqrt(alpha^2+beta^2+gamma^2))-1/4/gamma
})))
...
by.rZ <- lapply(rZs, function(rZ) {
    ...
    for(n in ns) {
        ...
        ## probs.gt <- cbind(probs.gt, mean(theta.hat^2> abs(x*y)))
        probs.gt <- cbind(probs.gt, mean(abs(term1)<abs(term2)))
        ## probs <- cbind(probs,median(x*y - theta.hat*(x+y) + theta.hat^2))
        ## probs <- cbind(probs,)
    }
    list(probs.full=probs.full,probs.gt=probs.gt)
})

by.rZ.full <- sapply(by.rZ, function(lst)lst[['probs.full']])
by.rZ.gt <- sapply(by.rZ, function(lst)lst[['probs.gt']])
...
n <- 1e4
mean(rnorm(n,sd=1/sqrt(n))^2 > abs(rnorm(n)))
...
[O(1/n) for t with df>=2, fails at t=1, 1.5. also fails for
beta[.2,.2]. but betas seem to be on the <.5 side but going to
.5 faster than 1/n.]
...
for(jj in 1:1e4) {
    ...
    ## stopifnot((A-B)*(A+B)<0)
    stopifnot(abs(A)<abs(B))
    ## stopifnot(B>0)
    stopifnot(abs(A+abs(B) - eig$val[1]) < 1e-8)
    stopifnot(abs(A-abs(B) - eig$val[n]) < 1e-8)
...
## from sherlock [moved to 210519 sim]
require(parallel)
...
p <- runif(1)
z0 <- ((1-p)/2)^(1/(1-p))
integrate(function(z)abs(z)^(-p),-z0,0)$val

## define response distribution unbounded near 0
dub <- function(z,p=1/2){z0 <- ((1-p)/2)^(1/(1-p)); abs(z)^(-p)*(abs(z)< z0)}
pub <- function(z,p=1/2){z0 <- ((1-p)/2)^(1/(1-p)); (1/2+z/(1-p)/abs(z)^p)*(abs(z)<=z0) + (z>z0)}
qub <- function(u,p=1/2) ((1-p)*abs(u-1/2))^(1/(1-p))*sign(u-1/2)
rub <- function(n,p=1/2)qub(runif(n),p=p)
## pub2 <- Vectorize(function(z)integrate(dub,-1,z)$val)
## curve(pub2,-.2,.2)
## curve(pub,add=TRUE,col=2)
## hist(qub(runif(1e6)),prob=TRUE)
## curve(dub,add=TRUE)

dd

## from sherlock -- big mean rather than mean of means [moved to 210519 sim]
## [ <0 for centered exponential as z]
## [the unbounded at 0 densities seem to be a problem]
require(parallel)
save.path <- '/mnt/d/data'
filelist <- dir(save.path)
filelist <- filelist[grep('^save[-0-9]+\\.RData',filelist)]
...
## rZ <- function(n)rnorm(n)
## rZ <- function(n)rt(n,df=2.25)
## rZ <- function(n)sample(c(-1,1),n,replace=TRUE)
## rZ <- function(n)rexp(n)-1
## rZ <- function(n)rub(n,p=.75)
rZ <- function(n)2*rbinom(n,1,1/2)-1/2

res <- mclapply(filelist[1:20],mc.cores=detectCores()-2,FUN=function(file) {
...
plot(names(mean.gt.vec),mean.gt.vec,col=2)


## see if counterexamples hold conditionally on s [seems so]

require(parallel)
rZ <- function(n)rbeta(n,.2,.2)-1/2
## rZ <- runif(n)-1/2
## rZ <- function(n)rnorm(n)
rZ <- function(n)rt(n,df=2.2)
## rZ <- function(n)rnorm(n) + 2*(2*rbinom(n,1,.5)-1)
## rZ <- function(n)sample(c(-1,1),n,replace=TRUE)
ns <- round(seq(5,200,len=10))
B <- 1e6

mean.gt <- mclapply(ns,mc.cores=detectCores()-2,FUN=function(n) {
## mean.gt <- lapply(filelist,FUN=function(file) {
    ## s <- runif(n)
    s <- rexp(n)
    Svec1 <- c(c(1/(s[1] - s[2]) - s[1]/sum(s^2), -1/(s[1] - s[2]) - s[2]/sum(s^2), -s[3]/sum(s^2), -s[4]/sum(s^2)), -s[4+seq_len(max(n+1-5,0))]/sum(s^2))
    Svec2 <- c(c(-s[1]/sum(s^2), -s[2]/sum(s^2), 1/(s[3] - s[4]) - s[3]/sum(s^2), -1/(s[3] - s[4]) - s[4]/sum(s^2)),  -s[4+seq_len(max(n+1-5,0))]/sum(s^2))
    SSmat <- (Svec1%*%t(Svec2) + t(Svec1%*%t(Svec2)))/2
    eig <- eigen(SSmat)
    eval.pos <- eig$val[1]; eval.neg <- eig$val[n]
    z <- matrix(rZ(n*B),nrow=n)
    proj.pos <- t(eig$vec[,1])%*%z
    proj.neg <- t(eig$vec[,n])%*%z
    mean(eval.pos*proj.pos^2 + eval.neg*proj.neg^2 < 0)
})
mean.gt <- simplify2array(mean.gt)
plot(ns,mean.gt); abline(h=1/2,lty=2)
...
SSmat <- (Svec1%*%t(Svec2) + t(Svec1%*%t(Svec2)))/2
eig <- eigen(SSmat)
evec1 <- eig$vec[,1]; evec2 <- eig$vec[,n]
lambda.a <- (eig$val[1]+eig$val[n])/2
lambda.b <- (eig$val[1]-eig$val[n])/2
lambda.a/lambda.b

## always <0 lambda.a term versus always positive projrest term
B <- 1e5
n <- 7
rZ <- rnorm
## rZ <- function(n)rt(n,df=2)
## rZ <- function(n).1*(rbeta(n,.2,.2)-1/2)
s <- rexp(n)
## s <- runif(n)
Svec1 <- c(c(1/(s[1] - s[2]) - s[1]/sum(s^2), -1/(s[1] - s[2]) - s[2]/sum(s^2), -s[3]/sum(s^2), -s[4]/sum(s^2)), -s[4+seq_len(max(n+1-5,0))]/sum(s^2))
Svec2 <- c(c(-s[1]/sum(s^2), -s[2]/sum(s^2), 1/(s[3] - s[4]) - s[3]/sum(s^2), -1/(s[3] - s[4]) - s[4]/sum(s^2)),  -s[4+seq_len(max(n+1-5,0))]/sum(s^2))
SSmat <- (Svec1%*%t(Svec2) + t(Svec1%*%t(Svec2)))/2
eig <- eigen(SSmat)
evec.pos <- eig$vec[,1]; evec.neg <- eig$vec[,n]
lambda.a <- (eig$val[1]+eig$val[n])/2
lambda.b <- (eig$val[1]-eig$val[n])/2
z <- matrix(rZ(n*B),nrow=n)
proj4.pos <- evec.pos[1:4]%*%z[1:4,]; proj4.neg <- evec.neg[1:4]%*%z[1:4,]
proj.pos <- evec.pos%*%z; proj.neg <- evec.neg%*%z
projrest.pos <- evec.pos[5:n]%*%z[5:n,]; projrest.neg <- evec.neg[5:n]%*%z[5:n,]
summary(lambda.b*as.numeric(projrest.pos^2-projrest.neg^2))
summary(lambda.a*as.numeric(proj.pos^2+proj.neg^2))
summary(lambda.b*as.numeric(projrest.pos^2-projrest.neg^2) + lambda.a*as.numeric(proj.pos^2+proj.neg^2))
## plot(lambda.b*as.numeric(projrest.pos^2-projrest.neg^2), lambda.a*as.numeric(proj.pos^2+proj.neg^2))
summary(as.numeric(lambda.a*as.numeric(proj.pos^2+proj.neg^2) + lambda.b*2*(proj4.pos*projrest.pos-proj4.neg*projrest.neg)))
plot(as.numeric(lambda.a*as.numeric(proj.pos^2+proj.neg^2)),as.numeric( lambda.b*2*(proj4.pos*projrest.pos-proj4.neg*projrest.neg)))

B <- 1e4
n <- 10
factor <- 3
vposrest <- rep(1/n,n)
vpos4 <- rep(sqrt((1-1/n)/4),4)
vnegrest <- factor*rep(1/n,n)
vneg4 <- rep(sqrt((1-factor^2/n)/4),4)
vposrest <- evec1[5:(n+4)]; vnegrest <- evec2[5:(n+4)]
vpos4 <- evec1[1:4]; vneg4 <- evec2[1:4]
rZ <- rnorm
## rZ <- function(n)rt(n,df=1.5)
## rZ <- function(n)30*(rbeta(n,.2,.2)-1/2)
## proj2s <- replicate(1e2, {
zrest <- matrix(rZ(n*B),nrow=n)
z4 <- matrix(rZ(4*B),nrow=4)
proj2s <- rbind((vposrest%*%zrest)^2 - (vnegrest%*%zrest)^2,(vpos4%*%z4)^2 -(vneg4%*%z4)^2)
## })
boxplot(t(proj2s)); abline(h=0,lty=2)


n <- 6
B <- 1e5
## rZ <- rnorm
rZ <- function(n)rt(n,df=3)
rZ <- function(n)3*(rbeta(n,.2,.2)-1/2)
s <- rexp(n)
Svec1 <- c(c(1/(s[1] - s[2]) - s[1]/sum(s^2), -1/(s[1] - s[2]) - s[2]/sum(s^2), -s[3]/sum(s^2), -s[4]/sum(s^2)), -s[4+seq_len(max(n+1-5,0))]/sum(s^2))
Svec2 <- c(c(-s[1]/sum(s^2), -s[2]/sum(s^2), 1/(s[3] - s[4]) - s[3]/sum(s^2), -1/(s[3] - s[4]) - s[4]/sum(s^2)),  -s[4+seq_len(max(n+1-5,0))]/sum(s^2))
SSmat <- (Svec1%*%t(Svec2) + t(Svec1%*%t(Svec2)))/2
...
## diffs <- replicate(1e2, {
z <- matrix(rZ(n*B),nrow=n)
median((evec1%*%z)^2 - (evec2%*%z)^2)
plot((evec1%*%z)^2, (evec2%*%z)^2)
points((evec2%*%z)^2, (evec1%*%z)^2,col=rgb(1,0,0,.1))

boxplot(diffs); abline(h=0,lty=2)
median(diffs)

## exchangeability, not conditioning on s
save.path <- '/mnt/d/data/'
filelist <- dir(save.path)
filelist <- filelist[grep('^save[-0-9]+\\.RData',filelist)]

ns.common <- numeric()
ns.common <- seq(10,100,len=10)
by.ns <- list()
for (file in filelist[1:20]) {
    load(paste0(save.path,file))
    ns <- sapply(1:length(by.n),function(j)nrow(by.n[[j]][[1]]$vec))
    print(ns)
    if(length(ns.common)>0 && ns.common!=ns) next
    ns.common <- ns
    by.ns <- c(by.ns,list(by.n))
}
by.n <- lapply(1:length(by.ns[[1]]), function(j)do.call(c,lapply(1:length(by.ns), function(k)by.ns[[k]][[j]])))
## rm(by.ns)

B <- length(by.n[[1]])
ns <- ns.common

## rZ <- function(n)rt(n,df=1)
## rZ <- function(n)runif(n,-1,1)
rZ <- function(n)(rbeta(n,.3,.3)-1/2)
proj <- simplify2array(lapply(1:length(ns), function(j) {
    z <- matrix(rZ(ns[j]*B),nrow=ns[j])
    proj.pos <- colSums(z * sapply(by.n[[j]],function(lst)lst$vectors[,1]))
    proj.neg <- colSums(z * sapply(by.n[[j]],function(lst)lst$vectors[,2]))
    cbind(proj.pos=proj.pos,proj.neg=proj.neg)
}))
plot(proj[,'proj.pos',1],proj[,'proj.neg',1],col=rgb(0,0,0,.1))
points(proj[,'proj.neg',1],proj[,'proj.pos',1],col=rgb(1,0,0,.1))
abline(0,1);abline(0,-1)
## [look exchangeable, but 210519 sim suggests not quite]

plot(ecdf(proj[,'proj.pos',1]))
lines(ecdf(proj[,'proj.neg',1]),col=2)

## exchangeability, not conditioning on s, trying n=5 case
n <- 4
B <- 1e4
## rZ <- function(n)rt(n,df=1)
rZ <- function(n)runif(n,-1,1)
...
evecs <- replicate(B, {
    ...
    SSmat <- (Svec1%*%t(Svec2) + t(Svec1%*%t(Svec2)))/2
    eig <- eigen(SSmat)
    ## c(proj.pos=z%*%eig$vec[,1],proj.neg=z%*%eig$vec[,n])
    cbind(pos=eig$vec[,1],neg=eig$vec[,n])
})
evec.pos <- evecs[,'pos',]; evec.neg <- evecs[,'neg',]
proj.pos <- colSums(z*evec.pos)
proj.neg <- colSums(z*evec.neg)
plot(proj.pos,proj.neg,col=rgb(0,0,0,1))
abline(0,1);abline(0,-1)

## idx <- identify(proj.pos,proj.neg);z[1:4,idx];evec.pos[1:4,idx];evec.neg[1:4,idx]

prod.pos <- z*evec.pos; prod.neg <- z*evec.neg
sig.pos <- rbind(colSums(prod.pos[1:2,]<0),colSums(prod.pos[3:4,]<0))
sig.neg <- rbind(colSums(prod.neg[1:2,]<0),colSums(prod.neg[3:4,]<0))
sig.pos <- apply(sig.pos,2,sort); sig.neg <- apply(sig.neg,2,sort)
sigs <- apply(cbind(t(sig.pos),t(sig.neg)),1,paste,collapse='')
color.map <- apply(with(list(pairs=unique(apply(apply(expand.grid(0:2,0:2),1,sort),2,paste,collapse=''))),expand.grid(pairs,pairs)),1,paste,collapse='')
color.map <- structure(rainbow(length(color.map)),names=color.map)
colors <- color.map[sigs]
plot(proj.pos,proj.neg,col=colors)
legend('topright',col=color.map,legend=names(color.map),pch=1)

idx <- identify(proj.pos,proj.neg);z[1:4,idx];z[1:4,idx]*evec.pos[1:4,idx];z[1:4,idx]*evec.neg[1:4,idx]

pos <- abs(evec.pos); neg <- abs(evec.neg)
idx <- rbinom(ncol(pos),1,1/2) + 1
for(j in 1:ncol(pos)) pos[idx[j],j] <- -pos[idx[j],j]
idx <- rbinom(ncol(pos),1,1/2) + 3
for(j in 1:ncol(pos)) pos[idx[j],j] <- -pos[idx[j],j]
idx <- rbinom(ncol(neg),1,1/2) + 1
for(j in 1:ncol(neg)) neg[idx[j],j] <- -neg[idx[j],j]
idx <- rbinom(ncol(neg),1,1/2) + 3
for(j in 1:ncol(neg)) neg[idx[j],j] <- -neg[idx[j],j]

dd


plot(colSums(evec.pos),colSums(evec.neg))
abline(0,1,col=2);abline(0,-1,col=2)
summary(abs(colSums(evec.pos)) - abs(colSums(evec.neg)))
## [seems abs(sum evec.pos) > abs(sum evec.neg) deterministically

points(proj.neg,proj.pos,col=rgb(1,0,0,.1))
mean(abs(proj.pos)<abs(proj.neg))

## idx <- which.max(proj.pos)
## identify(proj.pos,proj.neg)
points(proj.pos[idx.pos],proj.neg[idx.pos],col=3)
points(proj.pos[idx.neg],proj.neg[idx.neg],col=3,pch='x')

dev.new()

plot(z[,idx.pos],ylim=c(-.7,.7))
points(evecs[,'pos',idx.pos],col=2)
points(evecs[,'neg',idx.pos],col=3)
points((1:5)+.1,z[,idx.neg],pch='x')
points((1:5)+.1,evecs[,'pos',idx.neg],col=2,pch='x')
points((1:5)+.1,evecs[,'neg',idx.neg],col=3,pch='x')
abline(h=0)

lines(z[,idx.pos]*evecs[,'pos',idx.pos],col=2,type='o')
lines(z[,idx.pos]*evecs[,'neg',idx.pos],col=3,type='o')
lines(z[,idx.neg]*evecs[,'pos',idx.neg],col=2,lty=2,type='o')
lines(z[,idx.neg]*evecs[,'neg',idx.neg],col=3,lty=3,type='o')
 
## patterns among first 4 eig vector components
n <- 4
B <- 1e3
## rZ <- function(n)rt(n,df=1)
rZ <- function(n)runif(n,-1,1)
...
## rZ <- function(n)(rbeta(n,.1,.1)-1/2)
z <- matrix(rZ(n*B),nrow=n)
rS <- function(n)runif(n,0,3)
## rS <- rexp

s.lst <- matrix(rS(n*B),nrow=B)
s.lst[,1]  <- 2
s.lst[,3] <- 2
## s.lst[,4] <- 2
## s.lst[,2] <- s.lst[,1]+1
## s.lst[,4] <- s.lst[,3]+1
s.lst <- split(s.lst,1:B)
...
evec.pos <- evecs[,'pos',]; evec.neg <- evecs[,'neg',]
plot(evec.pos[1,],evec.pos[2,],asp=1);abline(-1,1,col=2);abline(1,1,col=2);abline(0,-1,col=2);abline(h=0,v=0)
min(evec.pos[2,])

curve(-sqrt(1/2-x^2),add=T)
...
## s <- runif(4); d <- runif(1)
## s[4] <- s[3]+d; s[2] <- s[1]+d
## s <- c(0,1,3,4) # unnorm pos evec is nice
## s0 <- runif(1); f <- runif(1)
## s <- c(s0,s0*f,s0,s0*f)
## s <- rep(runif(2),2)
## s <- c(0,1,0,1.1)
## s <- c(0,2,3,1)+10
...
Svec1 <- c(c(1/(s[1] - s[2]) - s[1]/sum(s^2), -1/(s[1] - s[2]) - s[2]/sum(s^2), -s[3]/sum(s^2), -s[4]/sum(s^2)), -s[4+seq_len(max(n+1-5,0))]/sum(s^2))
    Svec2 <- c(c(-s[1]/sum(s^2), -s[2]/sum(s^2), 1/(s[3] - s[4]) - s[3]/sum(s^2), -1/(s[3] - s[4]) - s[4]/sum(s^2)),  -s[4+seq_len(max(n+1-5,0))]/sum(s^2))
SSmat <- (Svec1%*%t(Svec2) + t(Svec1%*%t(Svec2)))/2
eig <- eigen(SSmat); eig.pos <- eig$vec[,1];eig.neg <- eig$vec[,n]
cbind(eig.pos,eig.neg)
eig.pos*sqrt(sum(s^2))
eig.neg*sqrt(sum(s^2))

s4s <- seq(-5,5,len=200)
ep <- sapply(s4s, function(s4) {
s <- c(0,1,0,s4)
Svec1 <- c(c(1/(s[1] - s[2]) - s[1]/sum(s^2), -1/(s[1] - s[2]) - s[2]/sum(s^2), -s[3]/sum(s^2), -s[4]/sum(s^2)), -s[4+seq_len(max(n+1-5,0))]/sum(s^2))
    Svec2 <- c(c(-s[1]/sum(s^2), -s[2]/sum(s^2), 1/(s[3] - s[4]) - s[3]/sum(s^2), -1/(s[3] - s[4]) - s[4]/sum(s^2)),  -s[4+seq_len(max(n+1-5,0))]/sum(s^2))
SSmat <- (Svec1%*%t(Svec2) + t(Svec1%*%t(Svec2)))/2
...
plot(s4s,abs(ep[1,]),asp=1)
for(j in 2:4)points(s4s,abs(ep[j,]),col=j)

dd

## [v1 and v2 are exchangeable, v3 and v4 are, and (v1,v2) and (v3,v4) are also, where (v1,...,v4) is the pos or neg evec]
## [when s4-s1=s3-s2 (s2-s1=s4-s3), negative eigenvector components have size 1/2]
## [eigenvectors invariant to change in norm of s]
## [when s=(s1,s2,s1,s2), pos evec is (s[2],s[1],s[2],s[1])/|s| in size]

dd


## with and without cross term, conditionally on s
n <- 20
B <- 1e5
rZ <- rnorm
## rZ <- function(n)rt(n,df=3)
...
projrest.pos <- evec.pos[5:n]%*%z[5:n,]; projrest.neg <- evec.neg[5:n]%*%z[5:n,]
proj4.pos <- evec.pos[1:4]%*%z[1:4,]; proj4.neg <- evec.neg[1:4]%*%z[1:4,]
full <- (proj.pos)^2 - (proj.neg)^2
without <- proj4.pos^2+projrest.pos^2 - proj4.neg^2 - projrest.neg^2
summary(as.numeric(abs(full-without)))
plot(full,without)
abline(0,1,col='red')
...
## n <- diffs[,'ns']; diffs <- diffs[,c('sq4','cross','sqrest')]

## combined <- aggregate(diffs,list(n=n),mean)
## with(combined,{
##     plot(n,sq4,type='l',ylim=range(subset(combined,select=c(sq4,cross,sqrest))))
##     abline(h=0,lty=2)
##     lines(n,cross,col=2)
##     lines(n,sqrest,,col=3)
##     lines(n,sq4+cross+sqrest,col=4,lty=2)
##     mean(sq4+cross+sqrest<0)
## })
...
## ggsave('ms/power_curves.png')

summary((subset(by.distr,estimator=='debiased')$value - subset(by.distr,estimator=='standard')$value)/subset(by.distr,estimator=='standard')$value)
...
B <- 1e4
n <- 20
z <- matrix(rbeta(n*B,.15,.39),nrow=n)
hist(colMeans(z))

pS <- function(q)pexp(q)
## pS <- function(q)pchisq(q,df=5)
## pS <- function(q)punif(q)
ns <- 10^(1:10)
k <- 1:1e5
B <- 1
plot(log(ns),sapply(ns, function(n)sum(pS(n/(B+k))^n/(B+k))))
...
n <- 50
ns <- 1:2
plot(0,type='n',xlim=c(0,5),ylim=c(0,1))
for(n in ns) {
    z <- matrix(rexp(n*B),nrow=n)-1
    lines(ecdf(abs(colMeans(z))),col=n)
    }




## 54. exportable function
...
begg.test <- function(y,v,...) {
    ...
    pval.standard <- 2*pnorm(sqrt(9*n/4)*abs(tau.hat),lower.tail=FALSE)
    pval.adjusted <- 2*pnorm(sqrt(n/(4/9-bias.hat))*abs(tau.hat),lower.tail=FALSE)
    return(list(pval.standard=pval.standard,pval.adjusted=pval.adjusted,tau.hat=tau.hat,bias.hat=bias.hat))
}
...
m <- v1%*%v2 +t(v1%*%v2)
eig <- eigen(m)
w1 <- matrix(eig$vectors[,1],ncol=1)
w2 <- matrix(eig$vectors[,n],ncol=1)
...
##         y <- (z[3]-z[4])/(s[3]-s[4])
##         theta.hat <- sum(z*s)/sum(s^2)
##         ## A <- x*y - theta.hat*(x+y) + theta.hat^2
##         ## runif(n)  - 1/n*(1+log(n))
##         c(x>theta.hat,y>theta.hat)
##     })
##     ## mean(As<0)
##     cov(t(As))[1,2]
## })
...
## checking variance of x.bar^2 1/n^2
require(parallel)
## B <- 1e3
## n <- 5
Bs <- unique(round(seq(1e2,7e3,len=1e2)))
ns <- round(seq(6,1000,len=20))
B <- 3e2
## by.n <- sapply(ns, function(n) {
by.n <- mclapply(ns, mc.cores=detectCores()-3,FUN=function(n) {
    var(replicate(B,mean(rnorm(n))^2))
})
by.n <- simplify2array(by.n)
plot(ns,by.n*ns^(2))


## probability min(x,y) is > a symmetric z is > 1/4 for independent x,y
B <- 1e5
rZ <- rnorm
rZ <- function(n)rbeta(n,.1,.1)-1/2
mean(pmin(rZ(B),rZ(B))>rnorm(B)/5)



## contour plots for dependence between covariance terms
require(parallel)
## B <- 1e3
n <- 1e2
## Bs <- unique(round(seq(1e2,7e3,len=1e2)))
...
## z <- matrix(runif(B*n,-10,10),ncol=n)
## a <- .1;z <- matrix(rbeta(B*n,a,a),ncol=n)-1/2
## z <- matrix(rbinom(B*n,1,.5)-1/2,ncol=n)*2
s <- matrix(runif(B*n,1,2),ncol=n)
## s <- matrix(rgamma(B*n,shape=.54),ncol=n)
theta.hat <- rowSums(z*s)/rowSums(s^2)
theta.ind <- rowSums(z[,5:n]*s[,5:n])/rowSums(s[,5:n]^2)
x <- (z[,1]-z[,2])/(s[,1]-s[,2])
y <- (z[,3]-z[,4])/(s[,3]-s[,4])
A <- ((s[,1]-s[,2])*(s[,3]-s[,4]))^2*(x-theta.ind)
B <- ((s[,1]-s[,2])*(s[,3]-s[,4]))^2*(y-theta.ind)
kde0 <- kde2d(A,B)
image(kde0)

require(ggplot2)
ggplot(data.frame(A=A,B=B),aes(x=A,y=B)) + geom_point() + stat_density2d(aes(colour = ..level..))




## Negative covariance bias term. Need ~1e5 reps to see bias, clearest
## with normal z and gamma s
require(parallel)
## B <- 1e3
## n <- 5
Bs <- unique(round(seq(1e2,7e3,len=1e2)))
ns <- round(seq(6,2e2,len=20))
B <- 1e5
## by.n <- sapply(ns, function(n) {
by.n <- mclapply(ns, mc.cores=detectCores()-3,FUN=function(n) {
    ## medians <- sapply(ns, FUN=function(n) {          
    ## z <- matrix(rnorm(B*n),ncol=n)
    z <- matrix(rcauchy(B*n),ncol=n)
    ## z <- matrix(runif(B*n,-10,10),ncol=n)
    ## a <- .1;z <- matrix(rbeta(B*n,a,a),ncol=n)-1/2
    ## s <- matrix(runif(B*n,0,2),ncol=n)
    s <- matrix(rgamma(B*n,shape=.54),ncol=n)
    theta.hat <- rowSums(z*s)/rowSums(s^2)
    theta.ind <- rowSums(z[,5:n]*s[,5:n])/rowSums(s[,5:n]^2)
    x <- (z[,1]-z[,2])/(s[,1]-s[,2])
    y <- (z[,3]-z[,4])/(s[,3]-s[,4])
    ## median(x*y - (x+y)*theta.hat)
    ## mean(    x*y - (x+y)*theta.hat < 0)-1/2
    ## A <- x*y - (x+y)*theta.ind
    ## B <- 1/(rowSums(s^2))^2*((x+y)*rowSums(z[,1:4]*s[,1:4])*(rowSums(s[,1:4]^2)-rowSums(s[,5:n]^2))+rowSums(z*s)^2)
    A <- x*y - (x+y)*theta.ind
    B <- (theta.ind-theta.hat)*(x+y) + theta.hat*2
    ## A <- x*y - (x+y)*theta.hat
    ## B <- theta.hat^2
    ## c(A=mean(A<0)-1/2, B=1-mean(abs(B)<abs(A)))
    mean((A<0) & (abs(B)<abs(A)))
    mean(A+B<0)-1/2
    ## mean((abs(B)>abs(A))[A<0])
    cov((z[,1]-z[,2])/(s[,1]-s[,2]) > theta.hat, (z[,3]-z[,4])/(s[,3]-s[,4]) > theta.hat)
    ## median(((z[,1]-z[,2])-theta.hat*(s[,1]-s[,2]))*((z[,3]-z[,4])-theta.hat*(s[,3]-s[,4]))*(s[,1]-s[,2])*(s[,3]-s[,4]))
    ## mean(((z[,1]-z[,2])-theta.hat*(s[,1]-s[,2]))*((z[,3]-z[,4])-theta.hat*(s[,3]-s[,4]))*(s[,1]-s[,2])*(s[,3]-s[,4])<0)-1/2
    ## x <- ((z[,1]-z[,2])-theta.hat*(s[,1]-s[,2]))*((z[,3]-z[,4])-theta.hat*(s[,3]-s[,4]))*(s[,1]-s[,2])*(s[,3]-s[,4])
})
...
plot(ns,by.n['A',]*ns,ylim=range(ns*t(by.n)))
lines(ns,by.n['B',]*ns,col=2)

plot(ns,ns*by.n/(1+log(ns)))


## 2. positive bias

## 2a. u-shaped z seems to cause positive bias: binomial, beta(.1,.1)
require(parallel)
## B <- 1e3
n <- 1e2
## Bs <- unique(round(seq(1e2,7e3,len=1e2)))
...
by.n <- mclapply(ns, mc.cores=detectCores()-3,FUN=function(n) {
    ...
    ## z <- matrix(rnorm(B*n),ncol=n)
    ## z <- matrix(runif(B*n,-10,10),ncol=n)
    a <- .2;z <- matrix(rbeta(B*n,a,a),ncol=n)-1/2
    ## z <- matrix(rbinom(B*n,1,.5)-1/2,ncol=n)*2
    s <- matrix(runif(B*n,1,2),ncol=n)
    ## s <- matrix(rgamma(B*n,shape=.54),ncol=n)
    theta.hat <- rowSums(z*s)/rowSums(s^2)
...
## 2b binomial z: bias term conditioning on z1,...,z4
require(parallel)
## B <- 1e3
n <- 1e2
Bs <- unique(round(seq(1e2,7e3,len=1e2)))
...
by.n <- mclapply(ns, mc.cores=detectCores()-3,FUN=function(n) {
    ...
    y <- (z[,3]-z[,4])/(s[,3]-s[,4])
    ## cov(x < theta.hat, y < theta.hat)
    C <- pmin(x,y)>theta.hat
    ## mean(C)
    ## mean((x<=theta.hat)*(y<=theta.hat))
    c(mean(C[z[,1]-z[,2]==0 & z[,3]-z[,4]==0])-1/2,
    c(mean(C[z[,1]-z[,2]==2 & z[,3]-z[,4]==2]),
    mean(C[z[,1]-z[,2]==-2 & z[,3]-z[,4]==-2]),
    ## mean(C[z[,1]-z[,2]==-2 & z[,3]-z[,4]==0]),
    ## mean(C[z[,1]-z[,2]==2 & z[,3]-z[,4]==0]),
    mean(C[z[,1]-z[,2]==-2 & z[,3]-z[,4]==2]))-1/4)
})
by.n <- simplify2array(by.n)
matplot(ns,t(by.n),pch='.',type='l',col=1)
abline(h=0,col=2)
...
## 2c. range of betas. Seems even slightly bimodal Z leads to unbounded bias.
n.rZ <- 5
rZs <- lapply(seq(.5,.1,len=n.rZ),function(a)function(n)rbeta(n,a,a)-1/2)
require(parallel)
## B <- 1e3
## n <- 5
## Bs <- unique(round(seq(1e2,7e3,len=1e2)))
...
colors <- gray.colors(n.rZ)
matplot(ns,ns*by.Z,type='l',col=colors,lty=1)
legend('topright',col=colors,lty=1,legend=seq(.5,.1,len=n.rZ))
## save.image('220124a.RData')



## 2c-1. with t distr. df=1, maybe df=1.25 cases seem to have
## unbounded bias. More pronounced with unif[0,1] than unif[1,2] for S
## distribution, seems to be some interaction.
n.rZ <- 5
Z.params <- seq(2,1,len=n.rZ)
rZs <- lapply(Z.params,function(df)function(n)rt(n,df=df))
require(parallel)
## B <- 1e3
## n <- 5
## Bs <- unique(round(seq(1e2,7e3,len=1e2)))
ns <- round(seq(6,3e2,len=20))
B <- 1e5
## by.n <- sapply(ns, function(n) {
## rZ <- rZs[[3]]
## by.n <- mclapply(ns, mc.cores=detectCores()-3,FUN=function(n) {
...
by.Z <- simplify2array(by.Z)
colors <- topo.colors(n.rZ)
matplot(ns,ns*by.Z,type='l',col=colors,lty=1)
abline(h=0)
legend('topleft',col=colors,lty=1,legend=Z.params)
## save.image('220124b.RData')


## 2c-2 considered for ms but too noisy
n.rZ <- 5
as <- c(2,1,.1)
rZs <- lapply(as,function(a)function(n)rbeta(n,a,a)-1/2)
require(parallel)
## B <- 1e3
## n <- 5
## Bs <- unique(round(seq(1e2,7e3,len=1e2)))
...
## by.n <- mclapply(ns, mc.cores=detectCores()-3,FUN=function(n) {
op <- par(mfrow=c(2,3))
for(a in as)curve(dbeta(x+1/2,a,a),-1/2,1/2,xlab='',ylab='')
## by.Z <- mclapply(rZs, mc.cores=detectCores()-3,FUN=function(rZ) {
for(a in as) {
    by.n <- mclapply(ns,mc.cores=detectCores()-3,FUN=function(n) {
        z <- matrix(rbeta(B*n,a,a)-1/2,ncol=n)
        s <- matrix(runif(B*n,1,2),ncol=n)
        theta.hat <- rowSums(z*s)/rowSums(s^2)
        x <- (z[,1]-z[,2])/(s[,1]-s[,2])
        y <- (z[,3]-z[,4])/(s[,3]-s[,4])
    ...
    by.n <- simplify2array(by.n)
    plot(ns,ns*by.n,type='l',xlab='',ylab='')
}
par(op)
...
## unbounded bias. More pronounced with unif[0,1] than unif[1,2] for S
## distribution, seems to be some interaction.[moved to manuscript.R]. But, not a scale family, no variances/expectations for some of these t's.
source('../begg2/misc.R')
n.rZ <- 5
...
## 2d FPR for 1. t distributions and 2. betas. Critical value appears
## to be around df=1.5, where FPR->1 as sample size increases for
## df<=1.5, at least for unif[1,2] S distribution.
source('misc.R')
## require(EnvStats)
tau <- function(z,s) {
    y <- z/s
...
q <- qnorm(1-.05/2)
rS <- rexp
rS <- function(n)runif(n,0,1)+1
n.rZ <- 5
Z.params <- seq(2.5,1,len=n.rZ) # 1
rZs <- lapply(Z.params,function(df)function(n)rt(n,df=df)) # 1
Z.params <- seq(.5,.1,len=n.rZ) # 2
rZs <- lapply(seq(.5,.1,len=n.rZ),function(a)function(n)rbeta(n,a,a)-1/2) # 2
distributions <- Z.params#structure(names(rSs),names=names(rSs))
ns <- c(25,75,150)#round(seq(1e1,1.5e2,len=20))
by.distr <- lapply(rZs, function(rZ) {
    by.n <- sapply(ns, FUN=function(n) {
        ...
        tau.stats <- simplify2array(tau.stats)
        mean(sqrt(9*n/4)*abs(tau.stats)>q)## ,power.debiased=mean(sqrt(n/(4/9-bias))*abs(tau.hat)>q),power.debiased.hat=mean(sqrt(n/(4/9-bias.hat))*abs(tau.hat)>q))
    })
})
out <- simplify2array(by.distr)
out <- round(out,3)
dimnames(out) <- list(n=ns,theta=Z.params)
out
## save.image('220124c.RData')
...
## 3 maximizing bias ratio
n <- 1e4
p <- runif(n)
a <- runif(n)
b <- a+runif(n)*(1-a)
## b <- runif(n)
4*max(p^2*(1-p)^2*(b-a)^2 / (p*a^2+(1-p)*b^2))
idx <- which.max(p^2*(1-p)^2*(b-a)^2 / (p*a^2+(1-p)*b^2))
c(p[idx],a[idx],b[idx])
...
max.S <- 1
n.bins <- max.S*n
m <- matrix(0,n.bins,n.bins)
for(i in 1:n.bins)m[i,i:n.bins] <- 0:(n.bins-i)
m <- (m+t(m))/n
## ev <- -eigen(m)$vec[,1]
## image(ev%*%t(ev))
## plot(ev)
## lm0 <- lm(ev ~ I((1:n)^2) + I(1:n))
## lines(fitted(lm0),col=2)
## eval <- eigen(m)$val
## eval[1]
## sum(abs(eval[-1]))
ps <- replicate(1e3,runif(n.bins))
ps <- t(ps)/colSums(ps)
quadform <- apply(ps, 1, function(p) 
## (t(p)%*%m%*%p)^2 / sum(p*( ((1:n.bins-1/2)/n)^2))
    t(p)%*%m%*%p
)
max(m)/2
max(quadform)
plot(ps[which.max(quadform),])

## rho looks concave on the simplex [update: is not], though it is
## indefinite on the whole space
n <- 6
max.S <- 1
## n.bins <- max.S*n
supports <- (0:(max.S*n))/n
n.supports <- length(supports)
m <- matrix(0,n.supports,n.supports)
for(i in 1:nrow(m))m[i,i:n.supports] <- 0:(n.supports-i)
m <- (m+t(m))/n
ps <- replicate(2e6,runif(n.supports))
ps <- t(ps)/colSums(ps)
rho <- function(p)(t(p)%*%m%*%p)#^2 / sum(p*supports^2)
rho <- function(p)(t(p)%*%m%*%p)^2 / sum(p*supports^2)
biasratios <- apply(ps,1,rho)
max(biasratios)
ps[which.max(biasratios),]
try <- (n+1)/(2*n+1)
try <- c(try,rep((1-try)/n,n))
c(observed.max=max(biasratios),try.max=rho(try))
...
n.cuts <- 1e1
cuts <- cut(biasratios,n.cuts)
colors <- gray.colors(n.cuts)[cuts]
plot(ps[,1],ps[,2],col=colors)
## p1*s1^2+p2*s2^2+(1-p1-p2)*s3^2=1, p1(s1^2-s3^2)+p2(s2^2-s3^2)=1-s3^2
## abline(a=1/supports[2]^2,b=-supports[1]^2/supports[2]^2,col=2)
legend('topright',col=gray.colors(n.cuts),pch=1,legend=levels(cuts))

pts <- identify(ps[,1],ps[,2],order=TRUE)
diff(biasratios[pts$ind[order(pts$order)]])


## checking definiteness [is indefinite]
n <- 2
max.S <- 1
## n.bins <- max.S*n
supports <- (0:(max.S*n))/n
...
for(i in 1:nrow(m))m[i,i:n.supports] <- 0:(n.supports-i)
m <- (m+t(m))/n
## ps <- replicate(1e5,runif(n.supports))
## ps <- t(ps)/colSums(ps)
rho <- function(p)(t(p)%*%m%*%p)^2 / sum(p*supports^2)
diffs <- replicate(1e4, {
p1 <- runif(n.supports); p1 <- p1/sum(p1)
p2 <- runif(n.supports); p2 <- p2/sum(p2)
## ts <- seq(0,1,len=50)
## plot(ts,sapply(ts,function(t)rho(t*p1+(1-t)*p2)),asp=1)
if((rho(p1)+rho(p2))/2  -   rho((p1+p2)/2) > 0.02) browser()
(rho(p1)+rho(p2))/2  -   rho((p1+p2)/2)
})


## checking candidate for argmax via monotonicity along rays [maybe: small counterex to monotonicity on rays at p=(3/4,1/4,0) for n=2. could still be max though]
n <- 3
max.S <- 1
## n.bins <- max.S*n
supports <- (0:(max.S*n))/n
...
ts <- seq(0,1,len=50)
diffs <- replicate(1e4, {
    p <- runif(n.supports); p <- p/sum(p)
    ## plot(ts,sapply(ts,function(t)rho(t*p+(1-t)*p.try)),asp=1)
    vals <- sapply(ts,function(t)rho(t*p+(1-t)*p.try))
    if(sum(diff(vals)>0)!=0)browser()
    sum(diff(vals)>0)
})
sum(diffs!=0)

plot(sapply(ts, function(t)rho(t*c(.75,.25,0)+(1-t)*try.max(2)))) ## increase at tail



## checking  conjectured max
try.max <- function(n){
    try <- (n+1)/(2*n+1)
    c(try,rep((1-try)/n,n))
}
...
for(j in 1:40) {
    ...
    ## ps[which.max(biasratios),]
    ## c(observed.max=max(biasratios),try.max=rho(try))
    print(max(biasratios) - rho(try.max(n)))
}
...
optim(par=try.max(n), fn=fn)$par - try.max(n)
## optim suggests conjectured is actual max at least locally




## 4 negative bias (poor FPR)

n <- 1e4
ns <- round(seq(1e5,1e6,len=10))
df <- 2.3
B <- 1e1
by.n <- sapply(ns, function(n)replicate(B ,var(rt(n,df=df)) - df/(df-2)))
...
diffs <- replicate(1e4, {
    sigma <- runif(n)+1
    y <- rnorm(n,sd=sigma)
    theta.hat <- sum(y/sigma^2)/sum(1/sigma^2)
    ## m <- sapply(1:2,function(k)sum(1/sigma^k))
    m2 <- sum(1/sigma^2)
    sigma.theta <- sqrt(1/sum(1/sigma^2))
    sigma.bar <- sqrt(sigma^2 - sigma.theta^2)
    ijkl <- sample(n,4)
    i <- ijkl[1]; j <- ijkl[2]; k <- ijkl[3]; l <- ijkl[4]
    W <- function(i,j)y[i]*(1/sigma.bar[i]*(1-1/m2/sigma[i]^2)+1/sigma.bar[j]/m2/sigma[i]^2)/(1/sigma.bar[i]-1/sigma.bar[j])*m2/sqrt(m2.del)
    m2.del <- sum(1/sigma[-c(i,j,k,l)]^2)
    theta.del <- function(idx) sum((y/sigma^2)[-idx])/sum(1/sigma^2)
    ## true <- ((y[i]-theta.hat)/sigma.bar[i] - (y[j]-theta.hat)/sigma.bar[j])*(sigma[i]-sigma[j])>0
    ## try <- (y[i]*(1/sigma.bar[i]*(1-1/m2/sigma[i]^2)+1/sigma.bar[j]/m2/sigma[i]^2)-y[j]*(1/sigma.bar[j]*(1-1/m2/sigma[j]^2)+1/sigma.bar[i]/m2/sigma[j]^2)-theta.del(c(i,j))*(1/sigma.bar[i]-1/sigma.bar[j]))*(sigma[i]-sigma[j])>0
    ## try <- (W(i,j)-W(j,i)-theta.del(c(i,j))*(1/sigma.bar[i]-1/sigma.bar[j]))*(sigma[i]-sigma[j]) > 0
    ## try <- (W(i,j)-W(j,i) - 1/m2*(y[k]/sigma[k]^2+y[l]/sigma[l]^2)*(1/sigma.bar[i]-1/sigma.bar[j]))*(sigma[i]-sigma[j]) > theta.del(c(i,j,k,l))*(1/sigma.bar[i]-1/sigma.bar[j])*(sigma[i]-sigma[j])
    ## try <- (W(i,j)-W(j,i) - 1/m2*(y[k]/sigma[k]^2+y[l]/sigma[l]^2)*(1/sigma.bar[i]-1/sigma.bar[j]))*(sigma[i]-sigma[j]) > theta.del(c(i,j,k,l))*(1/sigma.bar[i]-1/sigma.bar[j])*(sigma[i]-sigma[j]) 
    ## try <- (W(i,j)/(1/sigma.bar[i]-1/sigma.bar[j])+W(j,i)/(1/sigma.bar[i]-1/sigma.bar[j]) - 1/m2*(y[k]/sigma[k]^2+y[l]/sigma[l]^2)) < theta.del(c(i,j,k,l))
    ## try <- W(i,j)+W(j,i)- 1/sqrt(m2.del)*(y[k]/sigma[k]^2+y[l]/sigma[l]^2)  < theta.del(c(i,j,k,l))*m2/sqrt(m2.del)
    true <- (((y[i]-theta.hat)/sigma.bar[i] - (y[j]-theta.hat)/sigma.bar[j])*(sigma[i]-sigma[j])>0) * (((y[k]-theta.hat)/sigma.bar[k] - (y[l]-theta.hat)/sigma.bar[l])*(sigma[k]-sigma[l])>0)
    try <- (W(i,j)+W(j,i)- 1/sqrt(m2.del)*(y[k]/sigma[k]^2+y[l]/sigma[l]^2)  < theta.del(c(i,j,k,l))*m2/sqrt(m2.del)) && (W(k,l)+W(l,k)- 1/sqrt(m2.del)*(y[i]/sigma[i]^2+y[j]/sigma[j]^2)  < theta.del(c(i,j,k,l))*m2/sqrt(m2.del))
    try <- max( W(i,j)+W(j,i)- 1/sqrt(m2.del)*(y[k]/sigma[k]^2+y[l]/sigma[l]^2), W(k,l)+W(l,k)- 1/sqrt(m2.del)*(y[i]/sigma[i]^2+y[j]/sigma[j]^2) )  < theta.del(c(i,j,k,l))*m2/sqrt(m2.del)
    ## if(!all.equal(try,true))
    if(try!=true)
        browser()
    cat()
})
...
pairs <- replicate(1e2, {
    ...
    obs <- replicate(1e4, {
        ...
        W <- function(i,j)y[i]*(1/sigma.bar[i]*(1-1/m2/sigma[i]^2)+1/sigma.bar[j]/m2/sigma[i]^2)/(1/sigma.bar[i]-1/sigma.bar[j])*m2/sqrt(m2.del)
        theta.del <- function(idx) sum((y/sigma^2)[-idx])/sum(1/sigma^2)
        W0 <- W(i,j)+W(j,i)-1/sqrt(m2.del)*(y[k]/sigma[k]^2+y[l]/sigma[l]^2)
        W1 <- W(k,l)+W(l,k)-1/sqrt(m2.del)*(y[i]/sigma[i]^2+y[j]/sigma[j]^2)
        W1
    })
    ## mean(obs)
    ## ## summary(obs)
    true <- var(obs)
    ## 1/m2.del*(1/sigma[k]^2+1/sigma[l]^2) + m2^2/m2.del/(1/sigma.bar[i]-1/sigma.bar[j])^2*(   (sigma[i]/sigma.bar[i]*(1-1/m2/sigma[i]^2)+1/sigma[i]/m2/sigma.bar[j])^2 + (sigma[j]/sigma.bar[j]*(1-1/m2/sigma[j]^2)+1/sigma[j]/m2/sigma.bar[i])^2  )
    ## 1/m2.del*(1/sigma[k]^2+1/sigma[l]^2)+m2^2/m2.del/(1/sigma.bar[i]-1/sigma.bar[j])^2*((sigma[i]/sigma.bar[i])^2+(sigma[j]/sigma.bar[j])^2) + m2/m2.del*(-2+1/m2*(1/sigma[i]^2+1/sigma[j]^2))
    V0 <- 1/m2.del*(1/sigma[i]^2+1/sigma[j]^2+1/sigma[k]^2+1/sigma[l]^2) - 2*m2/m2.del + m2^2/m2.del/(1/sigma.bar[i]-1/sigma.bar[j])^2*((sigma[i]/sigma.bar[i])^2+(sigma[j]/sigma.bar[j])^2)
    V1 <- 1/m2.del*(1/sigma[i]^2+1/sigma[j]^2+1/sigma[k]^2+1/sigma[l]^2) - 2*m2/m2.del + m2^2/m2.del/(1/sigma.bar[k]-1/sigma.bar[l])^2*((sigma[k]/sigma.bar[k])^2+(sigma[l]/sigma.bar[l])^2)
    try <- V1
    c(true,try)
})
plot(pairs[1,],pairs[2,]); abline(0,1)
summary(abs(pairs[1,]-pairs[2,]))
...
pairs <- replicate(1e2, {
    ...
    obs <- replicate(1e4, {
        ...
        W1 <- W(k,l)+W(l,k)-1/sqrt(m2.del)*(y[i]/sigma[i]^2+y[j]/sigma[j]^2)
        ## theta.del <- function(idx) sum((y/sigma^2)[-idx])/sum(1/sigma^2)
        ## W(i,j)+W(j,i)- 1/sqrt(m2.del)*(y[k]/sigma[k]^2+y[l]/sigma[l]^2)
        ## c(y[i]/sigma[i]^2,W(i,j)) #a
        ## c(y[i]/sigma[i]^2,W(i,j),y[j]/sigma[j]^2,W(j,i)) #b
        c(W0,  W1      ) #c
        ## c(y[i]/sigma[i]^2,W(i,j),y[j]/sigma[j]^2,W(j,i),y[k]/sigma[k]^2,W(k,l),y[l]/sigma[l]^2,W(l,k),W(i,j)+W(j,i)-1/sqrt(m2.del)*(y[k]/sigma[k]^2+y[l]/sigma[l]^2), W(k,l)+W(l,k)-1/sqrt(m2.del)*(y[i]/sigma[i]^2+y[j]/sigma[j]^2)) #d
        ## c(y[i]+y[j]-1/sqrt(m2.del)*(y[k]/sigma[k]^2+y[l]/sigma[l]^2), y[k]+y[l]-1/sqrt(m2.del)*(y[i]/sigma[i]^2+y[j]/sigma[j]^2)) #e
    })
    V0 <- 1/m2.del*(1/sigma[i]^2+1/sigma[j]^2+1/sigma[k]^2+1/sigma[l]^2) - 2*m2/m2.del + m2^2/m2.del/(1/sigma.bar[i]-1/sigma.bar[j])^2*((sigma[i]/sigma.bar[i])^2+(sigma[j]/sigma.bar[j])^2)
    V1 <- 1/m2.del*(1/sigma[i]^2+1/sigma[j]^2+1/sigma[k]^2+1/sigma[l]^2) - 2*m2/m2.del + m2^2/m2.del/(1/sigma.bar[k]-1/sigma.bar[l])^2*((sigma[k]/sigma.bar[k])^2+(sigma[l]/sigma.bar[l])^2)
    cov01 <- -m2/m2.del*(2-1/m2*sum(1/sigma[c(i,j,k,l)]^2))
    ## cov(t(obs))[1,2]
    ## m2/sqrt(m2.del)/(1/sigma.bar[i]-1/sigma.bar[j])*(1/sigma.bar[i]*(1-1/m2/sigma[i]^2)+1/sigma.bar[j]/m2/sigma[i]^2)
    ## true <- cov(obs[1,],obs[2,])+cov(obs[3,],obs[4,]) #b
    ## try <- m2/sqrt(m2.del)*(1-1/m2*(1/sigma[j]^2+1/sigma[i]^2)) #b
    true <- cov(obs[1,],obs[2,]) #c
    try <- cov01 #c
    ## true <- cov(obs[9,],obs[10,]) #d
    ## try <- -1/sqrt(m2.del)*(cov(obs[1,],obs[2,])+cov(obs[3,],obs[4,]) +cov(obs[5,],obs[6,])+cov(obs[7,],obs[8,])) #d 
    ## true <- cov(obs[1,],obs[2,]) #e
    ## try <- -1/sqrt(m2.del)*4 #e
    true <- cor(obs[1,],obs[2,]) #f
    try <- cov01/sqrt(V0)/sqrt(V1) #f
    c(true,try)
})
plot(pairs[1,],pairs[2,]); abline(0,1)
summary(abs(pairs[1,]-pairs[2,]))
...
## 5ad integral for E(pnorm(min(W0,W1)))
n <- 5
B <- 1e3
pairs <- replicate(1e2, {
    sigma <- 1/(runif(n)*2+10)
    ijkl <- sample(n,4)
    i <- ijkl[1]; j <- ijkl[2]; k <- ijkl[3]; l <- ijkl[4]
    m2 <- sum(1/sigma^2)
    m2.del <- sum(1/sigma[-c(i,j,k,l)]^2)
    ...
    V <- c(V0,V1)
    cov01 <- -m2/m2.del*(2-1/m2*sum(1/sigma[c(i,j,k,l)]^2))
    ## V <- runif(2)*1e5 # poor performance at 1e5 factor
    ## V0 <- V[1]; V1 <- V[2]
    ## cov01 <- runif(1,0,sqrt(prod(V)))
    rho <- cov01/sqrt(V0)/sqrt(V1)
    cov.mat <- matrix(c(V0,cov01,cov01,V1),2)
    cov.mat.rt <- with(eigen(cov.mat), vectors%*%sqrt(diag(values))%*%t(vectors))
    Z <- matrix(rnorm(2*B),nrow=2)
    W <- cov.mat.rt%*%Z
    stats <- pnorm(apply(W,2,min))
    obs <- mean(stats)
    try <- sum(sapply(1:2, function(j) 1/sqrt(V[j])*integrate(function(u)pnorm(u)*pnorm(u/sqrt(1-rho^2)*(rho/sqrt(V[j])-1/sqrt(V[3-j])))*dnorm(u/sqrt(V[j])),-100,100)$val))
    c(obs,try)
})
plot(pairs[1,],pairs[2,]); abline(0,1)
summary(abs(pairs[1,]-pairs[2,]))
...
V1 <- 1/m2.del*(1/sigma[i]^2+1/sigma[j]^2+1/sigma[k]^2+1/sigma[l]^2) - 2*m2/m2.del + m2^2/m2.del/(1/sigma.bar[k]-1/sigma.bar[l])^2*((sigma[k]/sigma.bar[k])^2+(sigma[l]/sigma.bar[l])^2)
V <- c(V0,V1)
cc <- cov01 <- -m2/m2.del*(2-1/m2*sum(1/sigma[c(i,j,k,l)]^2))
rho <- cov01/sqrt(V0)/sqrt(V1)
beta <- 1/sqrt(1-rho^2)*(rho/sqrt(V)-1/sqrt(rev(V)))
gamma <- 1/sqrt(V)
Delta <- function(i,j)1/sigma[i]-1/sigma[j]
beta/gamma/sqrt(1+beta^2+gamma^2)
(cc-V)/sqrt(prod(V)-cc^2+rev(V)-cc^2/V+(cc^2/V+V-2*cc))
(cc-V)/sqrt(prod(V)+sum(V)-cc^2-2*cc)
cc-V[1]
-2*m2^2/m2.del*(1/sigma[i]-1/sigma[j])^(-2)
cc-V[2]
-2*m2^2/m2.del*(1/sigma[k]-1/sigma[l])^(-2)
sum(V)-2*cc
2*m2^2/m2.del*((1/sigma[i]-1/sigma[j])^(-2)+(1/sigma[k]-1/sigma[l])^(-2))
2*m2^2/m2.del*(1/Delta(i,j)^2+1/Delta(k,l)^2)
prod(V)-cc^2
2*(1/m2.del*sum(1/sigma[c(i,j,k,l)]^2)-2*m2/m2.del)*m2^2/m2.del*(1/Delta(i,j)^2+1/Delta(k,l)^2)+4*m2^4/m2.del^2/Delta(i,j)^2/Delta(k,l)^2
sum(V)-2*cc+prod(V)-cc^2
2*(1+1/m2.del*sum(1/sigma[c(i,j,k,l)]^2)-2*m2/m2.del)*m2^2/m2.del*(1/Delta(i,j)^2+1/Delta(k,l)^2)+4*m2^4/m2.del^2/Delta(i,j)^2/Delta(k,l)^2
(cc-V[1])/sqrt(sum(V)-2*cc+prod(V)-cc^2)
-1/Delta(i,j)^2/sqrt(1/2*(1/m2^2*sum(1/sigma[c(i,j,k,l)]^2)-2/m2+m2.del/m2^2)*(1/Delta(i,j)^2+1/Delta(k,l)^2)+1/Delta(i,j)^2/Delta(k,l)^2)

cyc <- sum(1/sigma[c(i,j,k,l)]^2)
1/m2^2*cyc-2/m2+m2.del/m2^2
cyc/m2^2-2/m2+(m2-cyc/n)/m2^2
m2.del
m2-cyc/n

(2*m2^2/m2.del^2*sum(1/sigma[c(i,j,k,l)]^2)+ 2*m2^2/m2.del-4*m2^2/m2.del  )*(1/Delta(i,j)^2+1/Delta(k,l)^2) +4*m2^4/m2.del/Delta(i,j)^2/Delta(k,l)^2

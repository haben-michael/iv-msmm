...
n <- 8
ij <- combn(n,2)
ij <- apply(ij,2,paste,collapse='')
ijkl <- expand.grid(ij,ij)
ijkl <- apply(ijkl,1,paste,collapse='')
ijkl <- strsplit(ijkl,split='')
counts <- sapply(lapply(ijkl,unique),length)
sum(counts==3)
6*choose(n,3)
sum(counts==4)
choose(n,2)*choose(n-2,2)
sum(sapply(ijkl, function(x)x[1]=='1' & x[2]=='2' & sum(length(unique(x)))==3))
2*(n-2)

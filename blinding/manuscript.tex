...
\begin{document}
The average treatment effect is a common summary of the effect of
a treatment on a continuously varying outcome. The ATE is the
difference in the outcome between the subpopulation receiving the
treatment and a control subpopulation,
[[]]

If the data consists only of the responses and the treatment
indicators, the empirical ATE is efficient, in the sense that it
achieves the lowest asymptotic variance among [[get class from tsiatis
book]]. When covariates are available, further efficiency gains are
attainable [[]]. Efficient estimators taking into account of
covariates are described in [[]].

\section{Method}
\subsection{}
Assume for the data the following model:% $(Y_i,X_i,A_i), i=1,\ldots,n$ be iid according to some
% joint distribution $\mathcal{P}.$ We leave $\matchcal{P}$ unspecified
% except to require

\begin{gather}
\begin{aligned}
  \label{eqn:model}
  (Y_1,X_1,A_1),\ldots,(Y_n,X_n,A_n) &\overset{iid}{\sim}\mathcal{P}\\
  A&\perp X\\
  \PP(A=1\mid X) = \PP(A=1) = 1-\PP(A=0)&= p
\end{aligned}
\end{gather}
for some law $\mathcal{P}$.
\begin{figure}[h!]
  \centering
\begin{tikzpicture}
  ...
  \end{tikzpicture}
\end{figure}
In the context of a randomized trial with two arms, $Y_i$ is interpreted as the outcome of interest, $X_i$ as a vector of
covariates, and $A_i$ as a binary indicator of treatment. The
independence of $A_i$ and $X_i$ reflects an assumption of a random
assignment mechanism. Beyond these restrictions on $\mathcal{P},$
certain regularity assumptions are also made on the [...].


The target of estimation is the average treatment effect,
$$
...
\]
By a Taylor expansion, the influence function for $\hat{\psi}$ is
\[
\]
That is,
\[
...
A short calculation shows that the mean of the criterion function
$U(Y_i,A_i;\psi)$ is 0 when $\psi=\psi_0$. The consistency and
asymptotic normality of $\hat{\psi}$ then follow from the central
limit theorem and [ref].

% General theory for $Z$-estimators provide that this estimator is
% consistent and asymptotically normal provided that the mean of the criterion
% function $U(Y_i,A_i;\psi)$ is 0 at the true value of $\psi,$
% say, $\psi_0.$
% By a Taylor expansion argument, the consistency and asymptotic normality of this estimator follow from:
\footnote{
  \begin{align*}
...
% \begin{proof}
  ...
  \end{align*}
% \end{proof}
If the data consisted only of the outcomes $Y_i$ and treatment
indicators $A_i$, estimator $\hat{\psi}$ is semiparametric efficient,
that is, its asymptotic variance $\Var U(Y_i,A_i;\psi_0)$ is not greater than
the asymptotic variance of any other regular asymptotically linear
estimator. [cite]

The availability of covariates provides an opportunity for further
efficiency gains. [cite]. The semiparametric efficient influence
function may be obtained by minimizing the variance of
\[
\]

over arbitrary measurable $h$, and is found to be [cite] [should we
re-derive this result here?],
...
\end{align*}
A drawback of estimators taking account of covariates is that the
 relationship of the covariates to the outcome must usually be modeled
consistently. For example, $\E0$ and $\E1$ in [ref] must be modeled in
order to obtain the semiparametric estimator. This modeling process introduces the possibility of bias
by an analyst [cite]. One solution is to specify in advance of seeing
the data the model
to be used. A disadvantage of this solution is [what disadvantage?
should any modeling decisions be made based on the data?] [Tsiatis
2008] presents a solution that allows an analyst to exercise fuller
range of modeling expertise. Briefly, each regression $\E0$ and $\E1$ is modeled
by separate teams or individuals, each with access only to data
corresponding to their arm. The resulting regression estimates may
then be combined in [[]] to obtain $\hat{\psi}$.

\subsection{}
We propose another approach, eliminating the regressions on treatment
in []. Specifically, by weighting the response
\[
  Y = \tilde{Y}\frac{p^A(1-p)^{1-A}}{(1-p)^Ap^{1-A}}=\tilde{Y}\left(\frac{p}{1-p}\right)^{2A-1}.
\]
the terms may be combined as
\[
\]
The criterion function [ref] may be rewritten as
\[ 
\]
...
An analyst given the weighted data may then model the regression on
covariates. The estimate may then be substituting in [] to obtain the
semiparametric estimate of the ATE. 

This approach avoids the need for
a separate analyst for each arm. Additionally, a single regression
estimate will often be more precise than two combined regression
estimates. [toy example?]

\subsection{}

In some fields it is common to regress out a variable of interest $Y$ on
  ...
  [Example from epi literature]
This estimator is typically inefficient.
...
Since the estimator [[]] is efficient, the estimator [[]] is
inefficient when $p\neq 1/2,$ i.e., its asympototic variance is
larger. Specifically, the asymptotic variance of [[]] is
\[
\]
...
\[
  (1-2p)^2\Var(\E1-\E0),
\]
which is 0 if and only $p=1/2$ or the stratified ATEs
$\E1-\E0$ are constant.
\section{Simulation}
   As an illustration, suppose the response follows a linear
model,
\begin{align*}
  Y = \alpha A + \beta^T X + \gamma^T AX + \epsilon.
\end{align*}
The fixed parameters $\gamma$ represent interaction between $A$ and
$X$. The errors $\epsilon$ are assumed to have mean zero and the
variables $A,X,\epsilon$ are assumed mutually independent. In this
case, after a short calculation, the difference [[]] evaluates to
\[
  (1-2p)^2\gamma^T\Var(X)\gamma.
  \]
When there are no interactions, $\gamma=0,$ the difference vanishes,
since in this case the stratified ATEs $\E1-\E0$ are constant. By
another short calculation, the asymptotic relative efficiency of
$\hat{\beta_1}$ to $\hat{\psi}$ is
\[
  1 + \frac{(1-2p)^2\gamma^T\Var(X)\gamma}{\frac{\Var{(\epsilon)}}{p(1-p)}+\gamma^T\Var(X)\gamma}.
\]
See Fig. [[]]. 
